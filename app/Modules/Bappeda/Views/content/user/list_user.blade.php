        ...
        <div class="content-wrapper">
            <div class="content-header row">
                ...
                <div class="content-header-right col-md-3 col-12 mb-2">
                    <a href="{{ base_url('bappeda/user/add') }}" class="btn btn-block btn-info"><i
                                                ...
                                                @foreach ($user as $key => $val)
                                                    <tr>
                                                        ...
                                                        <td nowrap align="center" width="100">
                                                            <a href="{{ base_url('bappeda/user/edit/' . encode($val->id_user)) }}"
                                                            ...
                                                            <button type="button" onclick="hapusData(this, true)"
                                                                data-link="{{ base_url('bappeda/user/delete/' . encode($val->id_user)) }}"
                                                        ...
                                                        <td nowrap width="170"><span
                                                                class="badge badge-sm badge-{{ $val->id_role == 1 ? 'info' : ($val->id_role == 2 ? 'danger' : 'success') }} w-100">{{ strtoupper($val->nama_role) }}</span>
                                                        </td>
                                                        <td>{{ text_uc($val->nama_user) }}</td>
                                                        <td>{{ $val->username }}</td>
                                                    </tr>
                                                @endforeach
                                            </tbody>
...
@push('js_script')
    ...
    <script>
        function changeActive(data) {
            ...
            $.get("{{ base_url('bappeda/user/active/') }}/" + act + '/' + id,
                function(dt) {
