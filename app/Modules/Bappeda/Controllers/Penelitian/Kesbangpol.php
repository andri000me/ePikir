...
namespace App\Modules\Bappeda\Controllers\Penelitian;
...
class Kesbangpol extends BaseController
{
    ...
    public function rplMasuk()
    {
        ...
        $this->v_data['active']     = '2.1.1';

        return views('content/penelitian/kesbangpol/rpl_masuk', 'Bappeda', $this->v_data);
    }

    public function rplProses()
    {
        $this->v_data['status']     = '2';
        $this->v_data['active']     = '2.1.2';

        return views('content/penelitian/kesbangpol/rpl_proses', 'Bappeda', $this->v_data);
    }
    ...
    public function rplSelesai()
    {
        ...
        $this->v_data['active']         = '2.1.3';

        return views('content/penelitian/kesbangpol/rpl_selesai', 'Bappeda', $this->v_data);
    }
    ...
    public function getDataRpl($status = 1, $table = '')
    {
        ...
        if ($post) {
            ...
            foreach ($fetch_data as $row) {
                ...
                data-link="' . base_url('bappeda/penelitian/kesbangpol/hapusdata/' . encode($row->id_user_pemohon)) . '" 
                data-table="' . $table . '" 
    ...
    public function ubahStatusRpl($id = null, $status = 2, $msg_reason = '')
    {
        if ($id != null) {
            ...
            if ($status == 2) {
                ...
                $msg_add = "permohonan surat rekomendasi penelitian Anda sudah *" . $info_status . "*/ dan sedang ditindak lanjut oleh Admin Bappeda Kabupaten Magelang mulai tanggal " . formatTanggalTtd(date('Y-m-d')) . ".";
            }
            if ($status == 3) {
                $info_status = 'disetujui';
                $msg_add = "permohonan surat rekomendasi penelitian Anda sudah *" . $info_status . "*/ oleh Admin Bappeda Kabupaten Magelang mulai tanggal " . formatTanggalTtd(date('Y-m-d')) . ". Selanjutnya pemohon untuk segera mengambil *Surat Rekomendasi Penelitian*/ di kantor Bappeda Kabupaten Magelang dengan membawa berkas kelengkapan sesuai yang diupload di website.";
            }
            if ($status == 4) {
                $info_status = 'ditolak';
                $msg_add = "mohon maaf permohonan surat rekomendasi penelitian Anda *" . $info_status . "*/ oleh Admin Bappeda Kabupaten Magelang. Berikut detail penjelasannya:\n" . $msg_reason . "\nSilahkan bisa mengajukan ulang sesuai ketentuan.";
            }
